...
import { forecastEntries } from "@/lib/persona";

export function ForecastCard() {
  return (
    <section
      id="previsao-de-recebimento"
      aria-label="Previsão de recebimento"
    ...
    >
      <header className="flex items-center gap-2.5">
        <span className="inline-flex size-9 items-center justify-center rounded-full bg-inset text-ink">
          <TrendUp size={18} weight="bold" aria-hidden />
        </span>
        <div>
      ...
      <ol className="flex flex-col divide-y divide-line">
        {forecastEntries.map((entry) => (
          <li
            key={entry.id}
            className="relative flex flex-wrap items-start gap-3 rounded-md py-3 transition-colors duration-(--motion-fast) first:pt-0 last:pb-0 hover:bg-inset focus-within:bg-inset"
          >
            <Avatar
              name={entry.client}
            ...
            <div className="min-w-0 flex-1 basis-40">
              ...
              <p className="text-xs text-ink-soft">{entry.conditionLabel}.</p>
            </div>
