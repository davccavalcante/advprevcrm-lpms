...
- Infrastructure: Supabase (database, auth, storage) in South America, São Paulo since 2026-09-01, file handling, and external integrations. Fourteen tables with row level security on every one of them, twenty nine policies and two private buckets, versioned in `supabase/migrations/0001_initial_schema.sql`.
...
Measured on 2026-09-01: two of the three environments exist in practice and they share one database project, development and production, which is the divergence from the paragraph above and is recorded in `AUDIT.md` of that date rather than presented as satisfied. Production is live and its publication is proved at the address itself: the deploy asserts that the root redirects a browser without a session, that the sign in screen answers, and that an interior Brazilian Portuguese URL redirects, so a build that removed the authenticated door fails the check instead of passing it.
