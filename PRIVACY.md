...
Account profile, added 2026-08-21: the editable account record, name, electronic address, password and photo, lives under `data/`, outside version control. The password is stored only as a salted scrypt hash and never in clear, password material never enters the change log, and the photo is served by an application route from `data/`, never from a public asset folder, because a personal photo is personal data. Every profile change appends an immutable log line with author, moment and values before and after.
